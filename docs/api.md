...
| `PUT /config` | `{"patch": {...}}` or `{"yaml": "..."}` | `{saved, local, local_yaml, restart_required}` | deep-merge into `config/local.yaml` (a `null` leaf removes an override) or replace the file; validated before writing |
