...
| [api.md](api.md) | The local HTTP API (chat, speak, transcribe, trigger) |
...
Quick start: `conda activate alveus && alveus doctor --warm && alveus chat`, then say **"Aurea, …"** with `alveus talk` or the `alveus` systemd service.
